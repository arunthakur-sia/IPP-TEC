# TEC Innovation Program — Agent Platform

A Next.js implementation of the two coaching agents described in *TEC Innovation Program – AI Agent Build-out Plan*:
...
The app's data layer (every idea, assessment, prototype plan, mentor/coach review, pitch run, mock jury turn, and audit log entry) lives in Postgres via Supabase (`src/lib/supabase/client.ts`, `src/lib/db/queries/*.ts`). Set these in `.env.local`:
...
Open http://localhost:3000 — you'll land on `/login`. The app seeds five demo accounts into the `users` table on first run (one per role — participant, mentor, coach, program office, jury), all sharing one password out of the box: **`TecDemo#2026`** (override with `DEMO_ACCOUNT_PASSWORD` in `.env.local` before first run if you want a different one). Sign-in is real (email + password, checked server-side against a hashed password, session cookie signed and `httpOnly`) — but there's still no self-service sign-up and no real SSO; these are still the same five fixed seeded accounts, just with real credentials instead of a role-switcher dropdown (see *What's intentionally stubbed* below):

| Role | Email |
|---|---|
| Participant | participant@pilot.tec.gov.ae |
| Mentor | mentor@pilot.tec.gov.ae |
| Coach | coach@pilot.tec.gov.ae |
| Program office | program-office@pilot.tec.gov.ae |
| Jury | jury@pilot.tec.gov.ae |

To act as a different role, sign out (top bar) and sign back in with a different account.
...
- Net effect: ranges and exact counts (e.g. "rate all six dimensions," "1-5") are enforced by the prompt instruction and a `.describe()` hint, not by the JSON schema — `lib/scoring/*.ts` clamps ratings to 1-5 defensively before using them, since the schema can no longer guarantee that range.
...
**Orchestration.** Each agent is a LangGraph `StateGraph` whose nodes match the plan's own named stages exactly (`intake → clarify → assess → critique → verdict → prototype_plan → mentor_review → closed` for Agent 1; the equivalent 9-stage graph for Agent 2). Human-in-the-loop points (the clarify Q&A loop, mentor review, mock jury, coach review) use LangGraph's `interrupt()`/`Command(resume)` — the graph genuinely pauses mid-run and resumes from the next HTTP request, not a simulation of it.
...
**Reliability.** Per the plan's core design principle, the *model* only ever returns per-dimension ratings and anchors — every weighted score, verdict, and hard rule (e.g. "strategic fit rated 1 forces Pivot") is computed in plain TypeScript (`lib/scoring/`), never by the model. Every structured call is validated against its zod schema again after the model returns (`lib/llm/structured.ts`), with one automatic retry on a schema mismatch. The critique pass (Agent 1) and the coherence stage (Agent 2) are separate model calls specifically to catch unsupported claims before anything reaches the participant.

**Data.** Postgres via Supabase (`supabase/migrations/0001_init.sql`, `lib/db/queries/*.ts`) — versioned rows for every assessment, prototype plan, mentor review, pitch run, and coach review, plus an append-only audit log. This is the durable source of truth; LangGraph's in-memory checkpointer is only a convenience for resuming mid-session within one server process (see the comment in `lib/agents/idea-validation/graph.ts` for the tradeoff and how it degrades — a server restart mid-session falls back to starting a fresh run from the durable Supabase state, not data loss).

## What's fully implemented

- Both agents run live end-to-end against Supabase + the real LLM gateway, not just unit-tested in isolation. Agent 1: intake through five real clarify turns, assess, critique, deterministic verdict, prototype plan, mentor-review brief. Agent 2: real deck upload/parse, structure, content, coherence, all six real mock-jury turns (persona rotation and turn counting verified correct across the self-loop), readiness, coach review, and a jury pack — including a mid-run recovery, when a dev-server hot-reload wiped the in-memory checkpoint mid-session, that resumed cleanly from the durable Supabase state without re-asking any already-answered mock jury question, which is exactly the resilience `lib/agents/idea-validation/graph.ts`'s comment describes
...
- All the screens named in the plan: Idea Workspace, Validation Session, Scorecard, Prototype Plan, Mentor Review, Pitch Studio, Structure Map, Coherence Report, Mock Jury, Readiness Dashboard, Coach Review, plus a Program Office dashboard and a Jury Pack view
...
- **SSO / TEC intranet identity** — the plan explicitly defers this to the pilot. Sign-in today is real (email + password, hashed and checked server-side, signed session cookie via `src/lib/auth/session.ts`), but it's still against five fixed seeded demo accounts — there's no self-service sign-up, no password reset, and no TEC-intranet identity behind it yet.
- **Mentor/coach assignment** — auto-assigns the single seeded mentor/coach rather than a real roster.
- **Evaluation harness** (golden set, calibration, CI regression gate) — not built; the plan scopes this as Stage 0 of the build plan, done with the program office and mentors, not something to fabricate.
- **Participant usefulness ratings / mentor & coach hours** — not collected, so the Program Office dashboard shows "—" for these rather than invented numbers.

## Branding

Colors are pulled from tec.gov.ae's own published stylesheet (`--color-tec-navy-*`, `--color-tec-gold-*` in `src/app/globals.css`). The logo is a placeholder text wordmark (`src/components/ui/TecWordmark.tsx`) — this build doesn't embed a scraped copy of the actual TEC / Government of Dubai crest. Drop the official asset at `public/brand/tec-logo.svg` and swap it into `TecWordmark.tsx` once you have it from TEC's brand team.

## Source document

`TEC Innovation Program - AI Agent Build-out Plan_2.docx` in the repo root is the spec this was built from.
