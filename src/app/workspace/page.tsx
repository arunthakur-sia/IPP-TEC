...
import { listAllIdeas, listIdeasForOwner, getIdeaAssessment } from "@/lib/db/queries/ideas";
...
import { getMentorReview } from "@/lib/db/queries/ideas";
...
export default async function WorkspaceListPage() {
  ...
  const rows = await Promise.all(
    ideas.map(async (idea) => {
      ...
      return { idea, assessment, phase: phaseForIdea(idea, mentorReview) };
    })
  );
            ...
            <Card key={idea.id} className="relative h-full transition-shadow hover:shadow-md">
              <Link href={`/workspace/${idea.id}`} className="block">
                <CardBody className="space-y-2">
                  ...
                  <p className="line-clamp-2 text-sm text-ink-500">{idea.canvas.problem}</p>
