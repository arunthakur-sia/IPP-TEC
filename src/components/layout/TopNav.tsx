...
import { common, roleLabels, t } from "@/lib/i18n/common";
...
export function TopNav({ currentUser }: { currentUser: User | null }) {
  ...
  const links: { href: string; label: string; roles?: User["role"][] }[] = [
    { href: "/workspace", label: t(common.ideas, locale) },
    { href: "/pitches", label: t(common.pitches, locale) },
    { href: "/program", label: t(common.programOffice, locale), roles: ["program_office", "mentor", "coach"] },
  ];
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        {currentUser && (
          <nav className="flex flex-1 items-center gap-1">
            {links
              .filter((l) => !l.roles || l.roles.includes(currentUser.role))
              .map((l) => (
                <Link
                  key={l.href}
                  href={l.href}
          ...
          {currentUser && (
            <>
              <span className="hidden text-sm text-ink-600 sm:inline">
                {currentUser.name} · {t(roleLabels[currentUser.role], locale)}
              </span>
              <button
                type="button"
                onClick={signOut}
